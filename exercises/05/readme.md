...
With the Addon installed to your Kyma cluster, we will provide an instance of the Redis service. 
The Redis service instance will bound to our previous Serverless Function created, the numbers-generator-service. 
...
- Bind the Redis service to the Serverless Function
- Consume Redis service to add and read keys inside the lambda

## Steps

### Create a new deployment file for our AddOn
...
Do not forget to inform the namespace when you are applying the file. If you do not define the namespace in the kubectl command, it will take the definition from the YAML file, and if it is not specified either, it will deploy your objects into the "default" namespace.
...
> Remember that we need to use *---* line as a separator between the object's declaration inside the YAML file
...
of parsing the result using a regular expression to filter the condition and status attributes.

The last condition in the status should be Ready equals True:

Great!! You have created the ServiceBinding object.
...
Let's get deeper into the details of the configuration.

Metadata section, set *numbers-redis-bindingfunction* as the Service Binding Usage object name.
The spec.serviceBindingRef and spec.usedBy fields are required. spec.serviceBindingRef area has been configured with the name of ServiceBinding that you have just created,
...
Select the numbers-generator-service, and click on it to see it configured.
...
* In the source code, we have created the following blocks
    ...
    * Object *client* calls the method *redis.createClient()* and use the ServiceBinding secret/credentials previously injected to this lambda; the variables HOST, PORT, PASSWORD creates a physical connection to our Redis instance  
    * At the end of the code block, the object *client* calls the method *set()* storing the last numbers generated into the Redis database. For the example purpose, the new Date() stores a key for this record.

The key/value inserted on Redis merely generates logging off the result of the insertion.
...
The next step will create a new lambda function that returns all the Redis service instance's data content.
...
Done!! The new service is already exposed!!!  
...
Open your /etc/hosts file again, and let's add the new numbers-history-service to the minikube IP.
...
Open your preferred browser and go the web app number-generator accessing the URL https://number-generator.kyma.local/.
Click on the button *Click Me* more than one time.

Please open a new browser window or a new tab and go to our recent history service, accessing the URL https://numbers-history-service.kyma.local/.
Immediately the service will return the history of the last numbers generated, and your window should look like this:

![kyma-runtime-virtual-event-05-04](kyma-runtime-virtual-event-05-04.png)

Congratulations!!! You have succeeded at the end of our exercise 5.